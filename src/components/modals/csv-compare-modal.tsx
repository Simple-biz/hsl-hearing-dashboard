...
export function CsvCompareModal({ onClose }: { onClose: () => void }) {
  ...
  const buildSyntheticData = () => {
    ...
    // This avoids discrepancies between client-side and server-side duplicate detection
    const entries = [
      ...results.newEntries,
      ...results.rescheduled,
      ...results.duplicates,
  ...
  const handleImportToRaw = async () => {
    if (!results) return;
    const entries = [...results.newEntries, ...results.rescheduled];
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          ...
          {results && (
            <>
              ...
              {hearingsCheckResult && importTarget === "hearings" && (
                <div className="space-y-3 mt-4 pt-4 border-t">
                  ...
                  {hearingsCheckResult.skippedRecords.length > 0 && (
                    <div className="rounded-lg border overflow-hidden">
                      <div className="flex items-center gap-2 px-3 py-1.5 bg-amber-50 dark:bg-amber-900/30 border-b">
                        <span className="h-2 w-2 rounded-full bg-amber-500" />
                        <p className="text-xs font-semibold text-amber-800 dark:text-amber-300">
                          Skipped — Cannot import (
                          {hearingsCheckResult.skippedRecords.length})
                        </p>
                      </div>
                      <div className="overflow-auto max-h-45">
                    ...
                    {hearingsCheckResult.newRecords.length > 0 &&
                      !hearingsImportResult?.newImported && (
                        ...
                        >
                          ...
                          Insert {hearingsCheckResult.newRecords.length} New
                        </Button>
                      )}
                    {hearingsCheckResult.rescheduledRecords.length > 0 &&
                      !hearingsImportResult?.reschedUpdated && (
                        ...
                        >
                          ...
                          Update {hearingsCheckResult.rescheduledRecords.length}{" "}
                          Rescheduled
                        </Button>
                      )}
                  </div>
