      ...
      {notes.length === 0 ? (
        ...
        <div className="border border-border rounded-lg overflow-hidden max-h-60 overflow-y-auto">
          {notes.map((n, i) => (
            <div
              key={i}
              className="px-4 py-3 border-b border-border last:border-0 hover:bg-muted/40"
            >
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-3 px-4 py-3 bg-muted/40 rounded-lg text-sm">
        ...
        <span className="text-muted-foreground">
          {new Date(h.hearing_date + "T00:00:00").toLocaleDateString("en-US", {
            month: "short",
            day: "numeric",
            year: "numeric",
          })}
        ...
        {h.rep_name && (
          <>
            <span className="text-muted-foreground">|</span>
            <span className="text-muted-foreground">{h.rep_name}</span>
          </>
        )}
        {h.mr_team_name && (
          <>
            <span className="text-muted-foreground">|</span>
            <span className="text-muted-foreground">{h.mr_team_name}</span>
          </>
        )}
      </div>
      ...
      <div>
        ...
        <p className="mt-3 text-xs text-center text-blue-600 dark:text-blue-400">
          ...
          <a href="/post-hearing" className="font-semibold hover:underline">
            Post Hearing Dashboard →
          </a>
        </p>
      </div>
    </div>
...
export function PostHrgModal({
  ...
  function handleDeadlineChange(id: number, deadline: string) {
    setHearings((prev) =>
      prev.map((h) =>
        h.id === id ? { ...h, post_hrg_deadline: deadline } : h,
      ),
    );
    if (selectedHearing?.id === id)
      setSelectedHearing((h) =>
        h ? { ...h, post_hrg_deadline: deadline } : h,
      );
  }
          ...
          {hearings.map((h) => (
            ...
            >
              ...
              <div className="text-[10px] text-muted-foreground">
                {new Date(h.hearing_date + "T00:00:00").toLocaleDateString(
                  "en-US",
                ...
                {h.post_hrg_deadline && (
                  <span className="ml-1.5 text-amber-600 dark:text-amber-400">
                    📅{" "}
                    {new Date(h.post_hrg_deadline).toLocaleDateString("en-US", {
                      month: "short",
                      day: "numeric",
                    })}
                  </span>
